...
static guint gegl_node_widget_signals[LAST_SIGNAL] = { 0 };
...
draw_node(EditorNode* node, cairo_t *cr, GeglEditor* editor)
{
  ...
  cairo_select_font_face(cr, NODE_FONT_FACE,
			 CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD);
  cairo_set_font_size(cr, PAD_FONT_SIZE);
  cairo_font_extents(cr, &fe);
  ...
  if(event->type == GDK_BUTTON_PRESS)
      ...
      if(pad)
	{
	  ...
	  if(pad->connected) {
	    ...
	    g_signal_emit (GEGL_EDITOR(widget), gegl_node_widget_signals[GRAPH_CHANGE_SIGNAL], 0);
	    }
      ...
      else
	{
	  ...
	  for(;node != NULL; node = node->next)
	    {
	      if(editor->px > node->x &&
	    		  editor->px < node->x+node->width	&&
	    		  editor->py > node->y &&
	    		  editor->py < node->y+node->height)
		{
...
gegl_editor_class_init(GeglEditorClass *klass)
{
  ...
  klass->graph_changed = NULL;

  gegl_node_widget_signals[GRAPH_CHANGE_SIGNAL] =
		  g_signal_new("graph_changed",
				  G_OBJECT_CLASS_TYPE((GObjectClass*)klass),
				  G_SIGNAL_RUN_FIRST,
				  G_STRUCT_OFFSET(GeglEditorClass, graph_changed),
				  NULL,
				  NULL,
...
void gegl_editor_add_connection(GeglEditor* self, gint from, gint to, const gchar* output, const gchar* input)
{
  ...
  g_signal_emit (GEGL_EDITOR(self), gegl_node_widget_signals[GRAPH_CHANGE_SIGNAL], 0);
}
